...
if __name__ == "__main__":
    print("Pruebas:\n")
    col_id = Column("id", DataType.INT, is_primary_key=True, has_index=True, index_type="hash")
    col_name = Column("nombre", DataType.VARCHAR, size=50, has_index=True, index_type="btree")
    col_precio = Column("precio", DataType.FLOAT)
    col_fecha = Column("fecha_registro", DataType.DATE)
    col_ubicacion = Column("ubicacion", DataType.ARRAY_FLOAT, has_index=True, index_type="rtree")
    ...
    print("\nProbando parse_value")
    val_int = parse_value("42", DataType.INT)
    print(f"INT: {val_int} (tipo: {type(val_int)})")
    
    val_float = parse_value("3.14", DataType.FLOAT)
    print(f"FLOAT: {val_float} (tipo: {type(val_float)})")
    
    val_varchar = parse_value("Hola Mundo", DataType.VARCHAR, 50)
    print(f"VARCHAR: {val_varchar} (tipo: {type(val_varchar)})")
    
    val_date = parse_value("2024-01-15", DataType.DATE)
    print(f"DATE: {val_date} (tipo: {type(val_date)})")
    
    val_array = parse_value("ARRAY[-12.06, -77.03]", DataType.ARRAY_FLOAT)
    print(f"ARRAY_FLOAT: {val_array} (tipo: {type(val_array)})")
    
    print("\nPrueba de serializacion y deserializacion ")
    # INT
    serialized_int = serialize_value(42, DataType.INT)
    deserialized_int = deserialize_value(serialized_int, DataType.INT)
    print(f"INT: {42} - {serialized_int.hex()} - {deserialized_int}")
    
    # VARCHAR
    serialized_varchar = serialize_value("Hola", DataType.VARCHAR, 20)
    deserialized_varchar = deserialize_value(serialized_varchar, DataType.VARCHAR, 20)
    print(f"VARCHAR: 'Hola' - {len(serialized_varchar)} bytes - '{deserialized_varchar}'")
    
    # ARRAY_FLOAT
    serialized_array = serialize_value((-12.06, -77.03), DataType.ARRAY_FLOAT)
    deserialized_array = deserialize_value(serialized_array, DataType.ARRAY_FLOAT)
    print(f"ARRAY_FLOAT: {(-12.06, -77.03)} - {len(serialized_array)} bytes - {deserialized_array}")
    
    print("\nTamanios de tipos\n")
    print(f"INT: {col_id.get_size()} bytes")
    print(f"VARCHAR[50]: {col_name.get_size()} bytes")
    print(f"FLOAT: {col_precio.get_size()} bytes")
    print(f"DATE: {col_fecha.get_size()} bytes")
    print(f"ARRAY_FLOAT: {col_ubicacion.get_size()} bytes")
