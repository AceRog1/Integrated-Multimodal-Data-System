...
if __name__ == "__main__":
    ...
    manager = TableManager(data_dir="test_data")
    ...
    print("Creando tabla restaurantes")
    table = manager.create_table("restaurantes", columns, "id", "hash")
    print(table)
    
    print(f"\nTamanio de registro: {table.get_record_size()} bytes")
    print(f"Columnas indexadas: {[col.name for col in table.get_indexed_columns()]}")
    
    #offsets de columnas
    print("\nOffsets de columnas:")
    for col in table.columns:
        print(f"  {col.name}: offset={table.get_column_offset(col.name)}, size={col.get_size()}")
    
    print(f"\nTablas existentes : {manager.list_tables()}")
    
    print("\nInfo de la tabla restaurantes:")
    info = manager.get_table_info("restaurantes")
    print(json.dumps(info, indent=2))
    
    print("\nRecargar desde disco")
    manager2 = TableManager(data_dir="test_data")
    print(manager2)
    
    print("\nEliminando tabla restaurantes")
    manager.drop_table("restaurantes")
    print(f"Tablas restantes: {manager.list_tables()}")
    
    #limpiamos directorio de prueba
